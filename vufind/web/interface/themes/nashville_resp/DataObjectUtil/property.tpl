...
{if ((!isset($property.storeDb) || $property.storeDb == true) && !($property.type == 'oneToManyAssociation' || $property.type == 'hidden' || $property.type == 'method'))}
	<div class="form-group" id="propertyRow{$propName}">
		...
		{if $property.type == 'stack'}
			<div class="panel-group" id="stack_{$property.label|escapeCSS}">
				<div class="panel panel-default {if !empty($property.expandByDefault)}active{/if}">
					<div id="stack_body_{$property.label|escapeCSS}" class="panel-collapse {if !empty($property.expandByDefault)}in{else}collapse{/if}">
		...
		{elseif $property.type == 'NashvilleSelfRegNav'}
			{literal}
			<style type="text/css">
				div#propertyRowNashvilleSelfRegNav div.w3-container div.w3-border {
					background: yellow;
				...
				div#propertyRowNashvilleSelfRegNav div.w3-container div.w3-border div.w3-grey {
					background: green;
					border: 1px solid #000;
					display: inline-block;
					height: 24px;
			...
			<div class="w3-container">
				<button id="cancel" class="btn btn-primary" type="button" onclick="alert('Cancelled!')">Cancel Account Creation</button>
			{if ($property.next != '0')}
				<button id="continue" class="btn btn-primary" type="button">Continue...</button>
			{/if}
				<div class="w3-border">
					<div class="w3-grey" style="width:{$property.progress}"></div>
				</div>
			</div>
		{elseif $property.type == 'card'}
			<div class="panel-group" id="card_{$property.label|escapeCSS}">
				<div class="panel panel-default {if !empty($property.expandByDefault)}active{/if}">
					<div id="card_body_{$property.label|escapeCSS}" class="panel-collapse {if !empty($property.expandByDefault)}in{else}collapse{/if}">
						<div class="panel-body">
							{if $property.instructions}
								<div class="alert alert-info">
									{$property.instructions}
